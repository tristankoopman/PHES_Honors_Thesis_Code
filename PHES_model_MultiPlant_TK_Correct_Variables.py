...
Beta                                                              , (Num_Plants_Installed) the gain to multiply the output of one plant to scale to more or less installed generation capacity
...
Surplus_REN                                                       , the amount of surplus renewable and nuclear generation this hour
Grid_Demand_Left                                                  , (New_D) the demand that still needs to be met for this hour after dispatching nuclear and renewables 
...
Energy_Generated                                                  , The Power generated by the PHES plant, if it generated electricty this hour
Energy_Stored                                                     , The Power consumed by the PHES plant, if it stored water this hour
...
def PumpedHydroPlant(Beta, AC, Running_Average, Surplus_REN, Grid_Demand_Left, Water_Lvl, scenario): #This is the main function that is called on in the other model
    
    if(Beta == 0): #When PHES capacity is 0, just return all 0's to save time
    ...
    Pumping_Threshold = capacities['PHES Pumping Threshold'][scenario] #PT (%) #Operation Thresholds for Running Average (Pumping)
    Generating_Threshold = capacities['PHES Generating Threshold'][scenario] #GT (%) #Operation Thresholds for Running Average  (Generating)
    ...
    Max_Gen = Water_Density*Water_Flow_Rate_Gen*G*Hydraulic_Head*Efficiency/1000000*Beta #(MW) Calcaulte the maximum generation power output 
    Max_Pump = Water_Density*Water_Flow_Rate_Pump*G*Hydraulic_Head*Efficiency/1000000*Beta #(MW) Calculate the maximum power consumption if storing water
    ...
    Energy_Generated = 0; #Amount of energy to generate 
    Energy_Stored = 0; #Amount of energy to be consumed by the PHES plant to pump water to upper reservoir
    ...
    if((Surplus_REN == 0) and (Gen_Check == 1) ):  #Check if the generating threshold is exceeded and there is no surplus renewable energy generated
        if(Water_Lvl > PHES_Water_Lvl_Min): #Check that the water level in the reservoir is above the minimum water level
            Energy_Generated = np.minimum(Max_Gen, Grid_Demand_Left) #Calculate amount of power to generate, either the maximum or the amount of demand left on the grid after nuclear and RE allocated
            ...
            for x in range(0,3600): #Generate for 1 hour (3600 seconds)
                temp, PHES_Energy_Output[x] = GenOneSecond(New_Water_Level, Energy_Generated/Beta, Efficiency) #Calculate amount of water released, energy generated, new water level for current second
            ...
            Energy_Generated = np.mean(PHES_Energy_Output)*Beta #Calculate power generated for the current hour
                
            New_State = 2 #Generating now 
            
            return New_State, Energy_Generated, Energy_Stored, New_Water_Level, New_Average #return the output variables, Energy_Stored will be 0 and the new_state will be 2 (Generating)
            
            
    #Check if we should pump
    if((Surplus_REN > 0) or (Pump_Check == 1)): #Check if there is a surplus of RE or the pumping threshold is crossed
        if(Water_Lvl < PHES_Water_Lvl_Max): #check if the water level is below the maximum water level
                        
            if(Surplus_REN > 0):    #IF there is a renewable surplus, pump up the surplus
                Energy_Stored = np.minimum(Max_Pump, Surplus_REN)  #Amount of power to consume is the lowest of the maximum or surplus of RE
            else: #Otherwise pump either the maximum amount or the surplus generation for the current hour (Available capacity)
                Energy_Stored = np.minimum(Max_Pump, AC-1) #Amount of power to consume is the lowest of the maximum or the available capacity
            ...
            for x in range(0,3600): #Pump for 1 hour (3600 seconds)
                ...
                temp, PHES_Energy_Consumed[x] = PumpOneSecond(New_Water_Level, Energy_Stored/Beta, Efficiency) #Calculate amount of water added, energy consumed, new water level for current second
            ...
            Energy_Stored = np.mean(PHES_Energy_Consumed)*Beta  ##Calculate power consumed for the current hour
            
            New_State = 1 #Pumping now 
            
            return New_State, Energy_Generated, Energy_Stored, New_Water_Level, New_Average #return the output variables, MW_To_BeGen will be 0 and the new_state will be 1 (Pumping)

    #All pumping and generating conditions failed
    New_State = 0 #Standby
    
    return New_State, Energy_Generated, Energy_Stored, New_Water_Level, New_Average #return the output variables, Energy_Stored and Energy_Generated will be 0 and the new_state will be 0 (Standby)
