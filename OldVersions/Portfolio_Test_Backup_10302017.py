...
Wind_On_Cap = 1900.0 #Original values for ref 97 MW
Wind_Off_Cap = 1600.0 #Original : 0 MW, have to do 1 MW so model will run
Sol_Cap = 1500.0   # 50MW
NG_Cap = 25190.0   # 15190 MW
Hydro_Cap = 5355.0 # 3355 MW
Oil_Cap = 8000.0   # 7015 MW
Nuc_Cap = 4000.0   # 4300 MW
PHES_Cap = 1143.0  # From wikipedia, can change/check later
...
while yr_num < 1: #5 because conting starts at 0 and only have the following years: 2011, 2012, 2013, 2014, 2015  
    ...
    PHES_Water_Lvl = 285.9 #Can be started at any value
    ...
    for k in range (0,len(Demand)-1): #Have to do minus 1 because starting at zero 
        ...
        PHES_State, PHES_Demand[k],  PHES_Pump_Demand[k], PHES_Water_Lvl, Running_Average_Demand = PumpedHydroPlant(Demand[k], Running_Average_Demand, Renewable_Generation, Renewables_Dispatched, New_D, NG_Limit, PHES_State, PHES_Water_Lvl)
...
if ENS_Warning == 0:
    ...
    result_data = {'Plant_Type': ['PHES','Onshore Wind','Off Shore Wind','Solar','NG','Hydro', 'Oil','Nuclear'],
                   'Capacity_MW':[PHES_Cap,Wind_On_Cap,Wind_Off_Cap,Sol_Cap,NG_Cap,Hydro_Cap,Oil_Cap,Nuc_Cap],
                   'Total_Energy_MWh':[np.average(PHES_Energy),np.average(W_On_Energy), np.average(W_Off_Energy), np.average(Sol_Energy),np.average(NG_Energy), np.average(Hydro_Energy), np.average(Oil_Energy),np.average(Nuc_Energy)],
                   'Total_Power_MW':[np.average(PHES_Pow),np.average(W_On_Pow), np.average(W_Off_Pow), np.average(Sol_Pow),np.average(NG_Pow), np.average(Hydro_Pow), np.average(Oil_Pow),np.average(Nuc_Pow)],
                   'Capacity_Factor':[np.average(PHES_CF),np.average(W_CF), np.average(W_Off_CF), np.average(Sol_CF), np.average(NG_CF), np.average(Hydro_CF), np.average(Oil_CF),np.average(Nuc_CF)] }
