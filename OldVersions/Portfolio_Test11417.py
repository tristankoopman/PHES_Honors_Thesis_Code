...
Wind_On_Cap = 97.0 #Original values for ref 97 MW
Wind_Off_Cap = 1000.0 #Original : 0 MW, have to do 1 MW so model will run
Sol_Cap = 50.0   # 50MW
NG_Cap = 25190.0   # 15190 MW
Hydro_Cap = 3355.0 # 3355 MW
Oil_Cap = 8000.0   # 7015 MW
Nuc_Cap = 4000.0   # 4300 MW
PHES_Cap = 1150.0  # 1150 MW
...
while yr_num < 5: #5 because conting starts at 0 and only have the following years: 2011, 2012, 2013, 2014, 2015  
    Demand = []
    W_sp1 = []
    W_sp2 = []
    W_sp3 = []
...
if ENS_Warning == 0:
    ...
    result_data = {'Plant_Type': ['Onshore Wind','Off Shore Wind','Solar','NG','Hydro', 'Oil','Nuclear'],
                   'Capacity_MW':[Wind_On_Cap,Wind_Off_Cap,Sol_Cap,NG_Cap,Hydro_Cap,Oil_Cap,Nuc_Cap],
                   'Total_Energy_MWh':[np.average(W_On_Energy), np.average(W_Off_Energy), np.average(Sol_Energy),np.average(NG_Energy), np.average(Hydro_Energy), np.average(Oil_Energy),np.average(Nuc_Energy)],
                   'Total_Power_MW':[np.average(W_On_Pow), np.average(W_Off_Pow), np.average(Sol_Pow),np.average(NG_Pow), np.average(Hydro_Pow), np.average(Oil_Pow),np.average(Nuc_Pow)],
                   'Capacity_Factor':[np.average(W_CF), np.average(W_Off_CF), np.average(Sol_CF), np.average(NG_CF), np.average(Hydro_CF), np.average(Oil_CF),np.average(Nuc_CF)] }
