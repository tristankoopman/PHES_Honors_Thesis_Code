...
Wind_On_Cap = 10000.0 #Original values for ref 97 MW
Wind_Off_Cap = 10000.0 #Original : 0 MW, have to do 1 MW so model will run
Sol_Cap = 10000.0   # 50MW
...
while yr_num < 1: #5 because conting starts at 0 and only have the following years: 2011, 2012, 2013, 2014, 2015  
    ...
    for k in range (0,len(Demand)-1): #Have to do minus 1 because starting at zero 
        ...
        PHES_State, PHES_Demand[k],  PHES_Pump_Demand[k], PHES_Water_Lvl, Running_Average_Demand = PumpedHydroPlant(PHES_Cap, Demand[k], Running_Average_Demand, Renewable_Generation, Renewables_Dispatched, New_D, NG_Limit, PHES_State, PHES_Water_Lvl)
