...
"""
''''issue with model right now is that the CF for Offshore wind with PHES is lower than without PHES.
we expect it to be higher. Also the overall energy from offshore wind is less with PHES included in the model. 
I am thinking this has something to do with the times at which the PHES is storing electricity. Or maybe it is storing electricity and 
not recording it as energy served by wind. 

TO DO: 1.Check why model not working
2. Ask tristina to send file he used to graph the area plots. '''
...
while scenario < len(Wind_On_Cap):
    ...
    if(np.mod(PHES_Cap[scenario], 1150)== 0): #Calculate the number of PHES plants to use. np.mod is the modulus operator. Ex: 5%2 = 1; or 4%2 = 0. (Calculates remainder)
        ...
        PHES_Cap[scenario] = 1150*Num_PHES_Plants[scenario] #Change the PHES_Cap to the rounded up capacity
    ...
    Winter_Energy = {'Nuclear':Nuc_Demand[1000:1168],'NG':NG_Demand[1000:1168],'Oil':Oil_Demand[1000:1168],'Hydro':Hydro_Demand[1000:1168],
                'Wind Onshore':W_Demand[1000:1168], 'Wind Offshore':W_Off_Demand[1000:1168],'Solar': Sol_Demand[1000:1168],
                'PHES':PHES_Demand_Generate[1000:1168]}
...
result_data_energy = {'Scenario': [s for s in Scenario_Names],
        ...
        'PHES_CF':[sPHES_CF[s] for s in range(len(Scenario_Names))],
...
print(Result)
'''
writer = pd.ExcelWriter('C:/Users/Owner/OneDrive/Documents/Papers/Portfolio Paper/Results/PHES_Results_1_7_2018.xlsx')
