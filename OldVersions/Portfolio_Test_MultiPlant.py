...
from Portfolio_Defs import All_Yrs, PHD, Nuc_Outages, User_Inputs
from PHES_model_MultiPlant import PumpedHydroPlant
import matplotlib as mpl
mpl.rcParams['font.size'] = 12.0 #Adjust font size


#Set the Capacities for Each Technology
Wind_On_Cap = User_Inputs['Wind Onshore Capacity'][0] #Original values for ref 97 MW
Wind_Off_Cap = User_Inputs['Wind Offshore Capacity'][0] #Original : 0 MW, have to do 1 MW so model will run
Sol_Cap = User_Inputs['Solar Capacity'][0]   # 50MW
NG_Cap = User_Inputs['Natural Gas Capacity'][0]   # 15190 MW
Hydro_Cap = User_Inputs['Hydro Capacity'][0] # 3355 MW
Oil_Cap = User_Inputs['Oil Capacity'][0]   # 7015 MW
Nuc_Cap = User_Inputs['Nuclear Capacity'][0]   # 4300 MW
PHES_Cap = User_Inputs['Pumped Hydro Capacity'][0]  # 1150 MW


if(np.mod(PHES_Cap, 1150)== 0): #Calculate the number of PHES plants to use. np.mod is the modulus operator. Ex: 5%2 = 1; or 4%2 = 0. (Calculates remainder)
    Num_PHES_Plants = PHES_Cap/1150 #Divide the PHES capacity by 1150 to get number of plants, 1 plant = 1150 MW generation capacity
else:
    Num_PHES_Plants = np.floor(PHES_Cap/1150)+1 #If the user defined capacity isnt perfectly divisiable by 1150, round up. (np.floor rounds a decimal down to the nearest integer. Ex: floor(3.5) = 3
    PHES_Cap = 1150*Num_PHES_Plants #Change the PHES_Cap to the rounded up capacity
...
while yr_num < 1: #5 because conting starts at 0 and only have the following years: 2011, 2012, 2013, 2014, 2015  
    ...
    for k in range (0,len(Demand)-1): #Have to do minus 1 because starting at zero 
        ...
        Renewables_Dispatched = Nuc_Cap*Nuc_Out[k] + Sol_Tot[k] + W_Tot[k] + W_Off_MWh[k] #Rennewable energy dispatched to meet grid demand for current hour
        ...
        PHES_State, PHES_Demand[k],  PHES_Demand_Consumed[k], PHES_Water_Lvl, Running_Average_AC = PumpedHydroPlant(Num_PHES_Plants, Available_Cap[k], Running_Average_AC,Surplus_RE, New_D, PHES_Water_Lvl)
...
ax = WeekWinter.plot(kind='area', color=colors,stacked=True, title='Generation Mix for a Winter Week (With PHES)')
